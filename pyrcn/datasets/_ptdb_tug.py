...
def _get_labels(X: np.ndarray, df_label: pd.DataFrame) -> np.ndarray:
    ...
    if X.shape[0] == labels.shape[0]:
        ...
        y[:, 0] = np.interp(np.arange(float(len(labels)), step=0.5),
                            xp=np.arange(len(labels), step=1), fp=labels[:, 0])
        y[:, 1] = np.interp(np.arange(float(len(labels)), step=0.5),
                            xp=np.arange(len(labels), step=1), fp=labels[:, 1])
        return y
    elif X.shape[0] == 2*labels.shape[0] - 1:
        y[1:1+2*len(labels)-1, 0] = np.interp(np.arange(float(len(labels) - 1),
                                                        step=0.5),
                                              xp=np.arange(len(labels), step=1),
                                              fp=labels[:, 0])
        y[1:1+2*len(labels)-1, 1] = np.interp(np.arange(float(len(labels) - 1),
                                                        step=0.5),
                                              xp=np.arange(len(labels), step=1),
                                              fp=labels[:, 1])
        return y
