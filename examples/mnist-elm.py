...
def elm_pca(directory):
    ...
    # split train test
    X_train, X_test, y_train, y_test = train_test_split(
        X[:train_size], y[:train_size], train_size=50000, random_state=42)
...
def elm_hidden_layer_size(directory):
    ...
    try:
        ...
        for hls in 784 * np.array(fan_out):
            ...
            logger.info('hidden_layer_size: {0}, score: {1}'
...
def elm_coates(directory):
    ...
    for filepath in glob.glob(os.path.join(directory,
        ...
        est_filepath = os.path.join(
            directory, 'est_coates-{0}.pickle'.format(filename))
        ...
        if (not os.path.isfile(csv_filepath)
            ...
            # save prediction
            np.savez_compressed(pred_filpath, X_test=X_test,
...
def elm_coates_stacked(directory):
    ...
    # refine best params
    logger.info('best parameters: {0} (score: {1})'
                .format(cv_best_params, cv.best_score_))
...
def silhouette_n_clusters(directory, *args, **kwargs):
    ...
    for n_clusters in k:
        ...
        dict_results['silhouette_original'].append(
            silhouette_score(X_train, clusterer.predict(X_train),
        ...
        dict_results['silhouette_variance_threshold'].append(
            silhouette_score(X_train, clusterer.predict(X_var_threshold),
                             metric='euclidean', random_state=42))
        ...
        dict_results['silhouette_pca'].append(
            silhouette_score(X_train, clusterer.predict(X_pca),
                             metric='euclidean', random_state=42))

        logger.info('n_clusters = {0}, pca kmeans score: {1}'
                    .format(n_clusters, dict_results['silhouette_pca'][-1]))
...
def silhouette_subset(directory, *args, **kwargs):
    ...
    for k in k_list:
        ...
        for subset_size in subset_sizes:
            ...
            # train preinit
            t = time.time()
            clusterer_init = KMeans(n_clusters=k, random_state=42, n_init=1,
            ...
            dict_results['silhouette_preinit'].append(
                silhouette_score(X_train, clusterer_init.predict(X_train),
                                 metric='euclidean', random_state=42))
            ...
            logger.info('silhouette (preinit) at subset size {1}: {0}'
                        .format(dict_results['silhouette_preinit'][-1],
...
def silhouette_features(directory, *args, **kwargs):
    ...
    for n_features in n_features_list:
        ...
        logger.info('pca silhouette at n_features={1:.0f}: {0}'
                    .format(dict_results['silhouette_pca'][-1], n_features))
