...
def plot_poster(directory, *args, **kwargs):
    ...
    plt.imsave(
        ...
        X[example_image_idx, :].reshape(28, 28), cmap=plt.cm.gray_r,
        format='png')
    ...
    pca_example = scale01(
        np.matmul(X[example_image_idx, :].reshape(1, -1),
                  np.matmul(
                      pca.components_.T, pca.components_))).reshape(28, 28)

    plt.imsave(
        os.path.join(os.environ['IMGPATH'], 'pca-component3.png'),
        pca_component,
    ...
    for idx in [0, 4, 9, 14, 19]:
        kmeans_centroid = scale01(
            clusterer.cluster_centers_[idx, ...]).reshape(28, 28)
        plt.imsave(
            os.path.join(os.environ['IMGPATH'],
                         'kmeans-centroid{0}.png'.format(idx)),
            kmeans_centroid, cmap=plt.cm.gray_r, format='png')

    # input weights
    T = np.load(
        os.path.join(os.environ['DATAPATH'], 'pca50+kmeans200_matrix.npy'),
        allow_pickle=True)
...
def plot_historgram(directory, *args, **kwargs):
    ...
    hist_fringe, bin_edges = np.histogram(
        X[:, idx_fringe[0] * 28 + idx_fringe[1]],
        bins=bins)
    hist_center, bin_edges = np.histogram(
        X[:, idx_center[0] * 28 + idx_center[1]],
        bins=bins)

    logger.info('validation sum hist_fringe: {0}, sum hist_center: {1}'
                .format(np.sum(hist_fringe / 1000),
    ...
    fig.savefig(
        os.path.join(os.environ['PGFPATH'], 'mnist-pixel-histogram.pgf'),
        format='pgf')
    # plt.show()
    return
...
def plot_var(directory, *args, **kwargs):
    ...
    fig.savefig(
        os.path.join(os.environ['PGFPATH'], 'mnist-pixel-variance.pgf'),
        format='pgf')
    # plt.show()
    return
...
def plot_image_min_var(directory, *args, **kwargs):
    ...
    fig.savefig(
        os.path.join(os.environ['PGFPATH'], 'mnist-img-min-var.pgf'),
        format='pgf')
    # plt.show()
    return
...
def plot_normalized(directory, *args, **kwargs):
    ...
    fig.savefig(
        os.path.join(os.environ['PGFPATH'], 'mnist-normalized.pgf'),
        format='pgf')
    # plt.show()
    return
...
def plot_pca(directory, *args, **kwargs):
    ...
    axs[2].imshow(
        np.resize(np.dot(X_avgfree[example_image_idx, ...], M), (28, 28)),
        cmap=plt.cm.gray_r, interpolation='none')
    ...
    fig.savefig(
        os.path.join(os.environ['PGFPATH'], 'mnist-pca-effects.pgf'),
        format='pgf')
    return
...
def plot_imbalance(directory):
    ...
    fig.savefig(
        os.path.join(os.environ['PGFPATH'], '{0}.pgf'.format(self_name)),
        format='pgf')
...
def plot_covariance(directory, *args, **kwargs):
    ...
    axs[1].set_xticklabels(np.append(['{0:.0f}'.format(x) if x == 0 else ''
                                      for x in
                                      np.arange(start=0, stop=n_components,
                                                step=28)],
    ...
    plt.imsave(os.path.join(directory, 'mnist-pca-components-db.png'),
               20 * np.log10(np.abs(cov_v_ordered.T) + 1.), cmap=plt.cm.gray_r,
               vmax=.5)
    return
...
def plot_img_cluster(directory, *args, **kwargs):
    ...
    for x in range(img_cluster_colors.shape[0]):
        for y in range(img_cluster_colors.shape[1]):
            img_cluster_colors[x, y, :] = list_cluster_colors[
                img_clusters[x, y]]
