...
def train_esn(base_input_to_node, base_node_to_node, base_reg, frame_length,
    ...
    try:
        ...
        for file in file_list[:7]:
            X, y = extract_features(file, sr=4000., frame_length=frame_length,
                                    target_widening=True)
            esn.partial_fit(X=X, y=y, postpone_inverse=True)
        X, y = extract_features(file_list[8], sr=4000.,
...
def extract_features(filename: str, sr: float = 4000., frame_length: int = 81,
                     target_widening: bool = True):
    ...
    y = librosa.util.frame(
        ...
        hop_length=1).T
    if target_widening:
        return X, np.convolve(y[:, int(frame_length / 2)], [0.5, 1.0, 0.5],
                              'same')
    else:
...
base_input_to_node = InputToNode(hidden_layer_size=500,
                                 input_activation='identity', k_in=5,
                                 input_scaling=14.6, bias_scaling=0.0,
                                 random_state=1)
base_node_to_node = NodeToNode(hidden_layer_size=500, spectral_radius=0.8,
                               leakage=0.5, k_rec=16, bidirectional=True,
                               random_state=1)
...
esn = base_esn
t1 = time.time()
Parallel(n_jobs=1, verbose=50)(delayed(train_esn)(base_input_to_node,
                                                  base_node_to_node, base_reg,
