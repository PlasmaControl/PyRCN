...
loss_function(esn, {'input_scaling': 0.1}, scaler=None, training_files=training_files, validation_files=validation_files)
with tqdm(total=len(training_files)) as pbar:
    for file_name in training_files:
        X, y = extract_features(file_name)
        np.random.seed(0)
        test_gpe = gpe(y_true=y[:, 0], y_pred=y[:, 0] + 100*np.random.rand(y[:, 0].shape[0]))
        test_vde = vde(y_true=y[:, 1], y_pred=y[:, 1])
        np.random.seed(0)
        test_fpe = fpe(y_true=y[:, 0], y_pred=y[:, 0] + 100*np.random.rand(y[:, 0].shape[0]))
        test_ffe = ffe(y_true=y[:, 0] * y[:, 1], y_pred=y[:, 0] * y[:, 1])
