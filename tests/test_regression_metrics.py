...
for k in range(10):
    _, y_true_mono[k] = make_regression(n_samples=10 * (k + 1), n_features=20,
                                        random_state=rng_true)
    _, y_pred_mono[k] = make_regression(n_samples=10 * (k + 1), n_features=20,
...
for k in range(10):
    _, y_true_multi[k] = make_regression(n_samples=10 * (k + 1), n_features=20,
                                         random_state=rng_true)
    _, y_pred_multi[k] = make_regression(n_samples=10 * (k + 1), n_features=20,
                                         random_state=rng_pred)


def test_mean_absolute_error() -> None:
    np.testing.assert_equal(
        pyrcn.metrics.mean_absolute_error(y_true=y_true_mono, y_pred=y_true_mono), 0)
    np.testing.assert_equal(
        np.greater(pyrcn.metrics.mean_absolute_error(y_true=y_true_mono,
                                                     y_pred=y_pred_mono), 0), True)
    np.testing.assert_equal(
        pyrcn.metrics.mean_absolute_error(y_true=y_true_mono, y_pred=y_pred_mono,
                                          sample_weight=sample_weight),
        sklearn.metrics.mean_absolute_error(y_true=np.concatenate(y_true_mono),
                                            y_pred=np.concatenate(y_pred_mono)))

    np.testing.assert_equal(
        pyrcn.metrics.mean_absolute_error(y_true=y_true_multi, y_pred=y_true_multi), 0)
    np.testing.assert_equal(
        np.greater(pyrcn.metrics.mean_absolute_error(y_true=y_true_multi,
                                                     y_pred=y_pred_multi), 0), True)
    np.testing.assert_equal(
        pyrcn.metrics.mean_absolute_error(y_true=y_true_multi, y_pred=y_pred_multi),
        sklearn.metrics.mean_absolute_error(y_true=np.concatenate(y_true_multi),
                                            y_pred=np.concatenate(y_pred_multi)))
    with pytest.raises(TypeError):
        pyrcn.metrics.mean_absolute_error(y_true=y_true_multi[0],
...
def test_mean_absolute_percentage_error() -> None:
    ...
    np.testing.assert_equal(
        ...
        sklearn.metrics.mean_absolute_percentage_error(
            y_true=np.concatenate(y_true_mono), y_pred=np.concatenate(y_pred_mono)))
    ...
    np.testing.assert_equal(
        ...
        sklearn.metrics.mean_absolute_percentage_error(
            y_true=np.concatenate(y_true_multi), y_pred=np.concatenate(y_pred_multi)))
    with pytest.raises(TypeError):
        pyrcn.metrics.mean_absolute_percentage_error(
...
def test_mean_squared_error() -> None:
    np.testing.assert_equal(
        pyrcn.metrics.mean_squared_error(y_true=y_true_mono, y_pred=y_true_mono), 0)
    np.testing.assert_equal(
    ...
    np.testing.assert_equal(
        pyrcn.metrics.mean_squared_error(y_true=y_true_multi, y_pred=y_true_multi), 0)
    np.testing.assert_equal(
        np.greater(pyrcn.metrics.mean_squared_error(
            y_true=y_true_multi, y_pred=y_pred_multi), 0), True)
    np.testing.assert_equal(
        pyrcn.metrics.mean_squared_error(y_true=y_true_multi, y_pred=y_pred_multi),
        sklearn.metrics.mean_squared_error(
            y_true=np.concatenate(y_true_multi), y_pred=np.concatenate(y_pred_multi)))
    with pytest.raises(TypeError):
        pyrcn.metrics.mean_squared_error(
...
def test_median_absolute_error() -> None:
    np.testing.assert_equal(
        pyrcn.metrics.median_absolute_error(y_true=y_true_mono, y_pred=y_true_mono), 0)
    np.testing.assert_equal(
    ...
    np.testing.assert_almost_equal(
        ...
        sklearn.metrics.median_absolute_error(
            y_true=np.concatenate(y_true_mono), y_pred=np.concatenate(y_pred_mono)), -1)
    ...
    np.testing.assert_equal(
        np.greater(pyrcn.metrics.median_absolute_error(
            y_true=y_true_multi, y_pred=y_pred_multi), 0), True)
    np.testing.assert_equal(
        pyrcn.metrics.median_absolute_error(y_true=y_true_multi, y_pred=y_pred_multi),
        sklearn.metrics.median_absolute_error(
            y_true=np.concatenate(y_true_multi), y_pred=np.concatenate(y_pred_multi)))
    with pytest.raises(TypeError):
        pyrcn.metrics.median_absolute_error(
...
def test_r2_score() -> None:
    np.testing.assert_equal(
        pyrcn.metrics.r2_score(y_true=y_true_mono, y_pred=y_true_mono), 1)
    np.testing.assert_equal(
        np.less(pyrcn.metrics.r2_score(
            y_true=y_true_mono, y_pred=y_pred_mono), 1), True)
    np.testing.assert_equal(pyrcn.metrics.r2_score(
        y_true=y_true_mono, y_pred=y_pred_mono, sample_weight=sample_weight),
        sklearn.metrics.r2_score(y_true=np.concatenate(y_true_mono),
                                 y_pred=np.concatenate(y_pred_mono)))
...
def test_max_error() -> None:
    np.testing.assert_equal(
        pyrcn.metrics.max_error(y_true=y_true_mono, y_pred=y_true_mono), 0)
    np.testing.assert_equal(np.greater(
        pyrcn.metrics.max_error(y_true=y_true_mono, y_pred=y_pred_mono), 1), True)
    np.testing.assert_equal(
        pyrcn.metrics.max_error(y_true=y_true_mono, y_pred=y_pred_mono),
        sklearn.metrics.max_error(y_true=np.concatenate(y_true_mono),
                                  y_pred=np.concatenate(y_pred_mono)))

    np.testing.assert_equal(
        pyrcn.metrics.max_error(y_true=y_true_multi, y_pred=y_true_multi), 0)
    np.testing.assert_equal(
        np.greater(pyrcn.metrics.max_error(y_true=y_true_multi,
                                           y_pred=y_pred_multi), 0), True)
    np.testing.assert_equal(
...
def test_mean_tweedie_deviance() -> None:
    np.testing.assert_equal(
        pyrcn.metrics.mean_tweedie_deviance(y_true=y_true_mono, y_pred=y_true_mono), 0)
    np.testing.assert_equal(
    ...
    np.testing.assert_equal(
        np.greater(pyrcn.metrics.mean_tweedie_deviance(
            y_true=y_true_multi, y_pred=y_pred_multi), 0), True)
    np.testing.assert_equal(
        pyrcn.metrics.mean_tweedie_deviance(y_true=y_true_multi, y_pred=y_pred_multi),
        sklearn.metrics.mean_tweedie_deviance(
            y_true=np.concatenate(y_true_multi), y_pred=np.concatenate(y_pred_multi)))
    with pytest.raises(TypeError):
        pyrcn.metrics.mean_tweedie_deviance(
