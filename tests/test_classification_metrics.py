...
def test_cohen_kappa_score() -> None:
    np.testing.assert_equal(
        pyrcn.metrics.cohen_kappa_score(y1=y_true_bin, y2=y_true_bin), 1)
    np.testing.assert_equal(
        np.less(pyrcn.metrics.cohen_kappa_score(y1=y_true_bin, y2=y_pred_bin), 1), True)
    np.testing.assert_equal(
        pyrcn.metrics.cohen_kappa_score(y1=y_true_bin, y2=y_pred_bin,
                                        sample_weight=sample_weight),
        sklearn.metrics.cohen_kappa_score(y1=np.concatenate(y_true_bin),
                                          y2=np.concatenate(y_pred_bin)))
    with pytest.raises(TypeError):
...
def test_jaccard_score() -> None:
    ...
    np.testing.assert_equal(
        np.less(pyrcn.metrics.jaccard_score(y_true=y_true_bin, y_pred=y_pred_bin), 1),
        True)
    np.testing.assert_equal(
        pyrcn.metrics.jaccard_score(y_true=y_true_bin, y_pred=y_pred_bin,
                                    sample_weight=sample_weight),
...
def test_matthews_corrcoef() -> None:
    np.testing.assert_equal(
        pyrcn.metrics.matthews_corrcoef(y_true=y_true_bin, y_pred=y_true_bin), 1)
    np.testing.assert_equal(
        np.less(
            pyrcn.metrics.matthews_corrcoef(y_true=y_true_bin, y_pred=y_pred_bin), 1),
        True)
    np.testing.assert_equal(
        pyrcn.metrics.matthews_corrcoef(y_true=y_true_bin, y_pred=y_pred_bin,
                                        sample_weight=sample_weight),
        sklearn.metrics.matthews_corrcoef(y_true=np.concatenate(y_true_bin),
                                          y_pred=np.concatenate(y_pred_bin)))
    with pytest.raises(TypeError):
        pyrcn.metrics.matthews_corrcoef(y_true=y_true_bin[0], y_pred=y_pred_bin[0])
...
def test_zero_one_loss() -> None:
    ...
    np.testing.assert_equal(
        np.less(pyrcn.metrics.zero_one_loss(y_true=y_true_bin, y_pred=y_pred_bin), 1),
        True)
    np.testing.assert_equal(
        pyrcn.metrics.zero_one_loss(y_true=y_true_bin, y_pred=y_pred_bin,
                                    sample_weight=sample_weight),
    ...
    np.testing.assert_equal(
        np.less(pyrcn.metrics.zero_one_loss(y_true=y_true_mlb, y_pred=y_pred_mlb), 1),
        True)
    np.testing.assert_equal(
...
def test_f1_score() -> None:
    ...
    np.testing.assert_equal(
        np.less(pyrcn.metrics.f1_score(y_true=y_true_bin, y_pred=y_pred_bin), 1), True)
    np.testing.assert_equal(
    ...
    np.testing.assert_equal(
        ...
        sklearn.metrics.f1_score(y_true=np.concatenate(y_true_mlb),
...
def test_fbeta_score() -> None:
    np.testing.assert_equal(
        pyrcn.metrics.fbeta_score(y_true=y_true_bin, y_pred=y_true_bin, beta=1), 1)
    np.testing.assert_equal(
    ...
    np.testing.assert_equal(
        ...
        sklearn.metrics.fbeta_score(y_true=np.concatenate(y_true_bin),
    ...
    np.testing.assert_equal(
        np.less(pyrcn.metrics.fbeta_score(
            y_true=y_true_mlb, y_pred=y_pred_mlb, average="micro", beta=1), 1), True)
    np.testing.assert_equal(
        pyrcn.metrics.fbeta_score(
            y_true=y_true_mlb, y_pred=y_pred_mlb, average="weighted", beta=0.5),
        sklearn.metrics.fbeta_score(y_true=np.concatenate(y_true_mlb),
                                    y_pred=np.concatenate(y_pred_mlb),
                                    average="weighted", beta=0.5))
    with pytest.raises(TypeError):
        pyrcn.metrics.fbeta_score(y_true=y_true_bin[0], y_pred=y_pred_bin[0], beta=0)
...
def test_precision_score() -> None:
    ...
    np.testing.assert_equal(
        ...
        sklearn.metrics.precision_score(y_true=np.concatenate(y_true_mlb),
                                        ...
                                        average="weighted"))
    with pytest.raises(TypeError):
        pyrcn.metrics.precision_score(y_true=y_true_bin[0], y_pred=y_pred_bin[0])
...
def test_balanced_accuracy_score() -> None:
    np.testing.assert_equal(
        pyrcn.metrics.balanced_accuracy_score(y_true=y_true_bin, y_pred=y_true_bin), 1)
    np.testing.assert_equal(
        np.less(pyrcn.metrics.balanced_accuracy_score(y_true=y_true_bin,
                                                      y_pred=y_pred_bin), 1), True)
    np.testing.assert_equal(
        pyrcn.metrics.balanced_accuracy_score(y_true=y_true_bin, y_pred=y_pred_bin,
                                              sample_weight=sample_weight,
                                              adjusted=True),
        sklearn.metrics.balanced_accuracy_score(y_true=np.concatenate(y_true_bin),
                                                y_pred=np.concatenate(y_pred_bin),
                                                adjusted=True))
    with pytest.raises(TypeError):
        pyrcn.metrics.balanced_accuracy_score(y_true=y_true_bin[0],
                                              y_pred=y_pred_bin[0])
...
def test_hamming_loss() -> None:
    ...
    np.testing.assert_equal(
        np.less(pyrcn.metrics.hamming_loss(y_true=y_true_bin, y_pred=y_pred_bin), 1),
        True)
    np.testing.assert_equal(
        pyrcn.metrics.hamming_loss(y_true=y_true_bin, y_pred=y_pred_bin,
                                   sample_weight=sample_weight),
        sklearn.metrics.hamming_loss(y_true=np.concatenate(y_true_bin),
                                     y_pred=np.concatenate(y_pred_bin)))

    np.testing.assert_equal(
        pyrcn.metrics.hamming_loss(y_true=y_true_mlb, y_pred=y_true_mlb), 0)
    np.testing.assert_equal(
        np.less(pyrcn.metrics.hamming_loss(y_true=y_true_mlb, y_pred=y_pred_mlb), 1),
        True)
    np.testing.assert_equal(
        pyrcn.metrics.hamming_loss(y_true=y_true_mlb, y_pred=y_pred_mlb),
        sklearn.metrics.hamming_loss(y_true=np.concatenate(y_true_mlb),
                                     y_pred=np.concatenate(y_pred_mlb)))
    with pytest.raises(TypeError):
